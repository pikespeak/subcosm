...
- [ ] **VIS-DEPTH**: Rework shell radius/spacing geometry so earlier/frozen days read distinctly all the way toward the core — the current `radius = Math.pow(0.85, idx)` (`src/engine/synthesis.ts`) crushes deep shells into a faint central blob. Per-shell brightness/size tuning allowed. Touches the engine contract `radius` → golden snapshots re-baselined and determinism tests updated; same `seed + genomeVersion` still reproduces identical output. Re-judged side-by-side vs `docs/subcosm-universe-mock.html`.
