...
- **App identity**: product name / branding is **Subcosm**; the globally-unique Devvit **app slug is `subcosm-universe`** ("Subcosm" alone was taken). The app is **already created** at `developers.reddit.com/apps/subcosm-universe`, scaffolded with the official **Phaser** template (`npm create devvit` → project dir `subcosm-universe`). Branding, logo, Devpost title stay "Subcosm".
- **Renderer decision (firm): Phaser is mandatory.** The paint/render layer uses **Phaser** (the user's firm decision + the scaffolded Phaser template), not raw Canvas 2D. This fits the architecture cleanly — `paint` is a swappable adapter behind the `Scene` seam; **synthesis stays pure and render-agnostic** (no Phaser import in `src/engine/synthesis`). It targets the "Best Use of Phaser" prize, and the official template resolves the earlier "WebGL-in-webroot" unknown. Trade-off accepted: Phaser learning curve + WebGL-specific caching (RenderTexture) instead of Canvas2D OffscreenCanvas.
