...
**Feasibility/scale note (2026-06-22):** The comment body IS present in the onCommentCreate trigger payload, so scanning for r/<sub> mentions is feasible; today the handler only accumulates counters and does NOT read the body — this idea adds read+scan there. Storage stays tiny: persist only sub→sub link edges (a small graph), never comment text/bodies — consistent with the no-stored-images / ~25-scalars-per-ring principle. The real cost is per-comment scanning CPU on very large subs (trigger-time), not storage; mitigate with cheap regex + dedup + a per-day cap. Participation check = lookup whether the target sub runs Subcosm.
**Scale resolution (scanning ALL comments):** stays cheap because (1) STORAGE: never store bodies — only deduped sub→sub edges (tiny graph); (2) per-comment cost is just a marginal regex on the already-firing onCommentCreate handler; (3) PARTICIPATION CHECK is the only real cost driver — do NOT look it up per mention/per comment. Instead dedup mentions into a per-day "mentioned subs" set, then resolve participation ONCE at tick against the EXISTING `subs:registry` ZSET (local Redis membership — no external Reddit call per mention). Cap mentions/day as a backstop.
**Requirements:** TBD
**Plans:** 0 plans
